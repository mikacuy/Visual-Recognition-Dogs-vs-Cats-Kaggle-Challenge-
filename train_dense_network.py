...
if (len(sys.argv)!=2):
  print("To train model please run \"python train_dense_network.py [directory for training images]\"")
  exit()
...
with graph.as_default():
  ...
  ##### variables 
  #Set 1 of [CONV->RELU->CONV->RELU->POOL]
  ...
  #Set 2 of [CONV->RELU->CONV->RELU->POOL]
  ...
  #Set 3 of [CONV->RELU->CONV->RELU->POOL]
...
with tf.Session(graph=graph) as session:
  ...
  #saver.restore(session, "./models/big_network_rm2/model_big_network_rm.ckpt")
  ...
  for epoch in range(num_epochs):
    ...
    for loading_batch in range(batch_sets):
        ...
        #print and graph current status every 21st iteration
				if ((step+1) % 21 == 0):
					feed_dict_validate={tf_valid_dataset : valid_dataset, tf_valid_labels: valid_labels}
					print ("Minibatch loss at batch:\t", loading_batch, "step:\t",step, "loss:\t", l)
					print ("Minibatch accuracy: %.1f%%" % accuracy(predictions, batch_labels))
					v_prediction, v_loss = session.run([valid_prediction, valid_loss], feed_dict=feed_dict_validate)
    ...
    plt.ylabel('Accuracy')
    plt.savefig('validation_accuracy_dense_network.png')

    plt.clf()
    plt.plot(x,y_valid_loss)
    plt.xlabel('Iterations')
    plt.ylabel("Validation cross entropy loss")
    plt.savefig('validation_loss_dense_network.png')

    plt.clf()
    plt.plot(x,y_training_loss)
    plt.xlabel('Iterations')
    plt.ylabel('Training minibatch loss')
    plt.savefig('training_miniloss_dense_network.png')
